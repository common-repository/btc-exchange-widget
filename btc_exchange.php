...
class btc_widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		wp_enqueue_script( 'btc_exchange_cookies_js', plugins_url('js.cookie-2.0.4.min.js', __FILE__), array(), false, true);
...
?>
		<script type="text/javascript">
			var currenciesJSON = {
				...
				"JPY":{"symbol":"&yen;","country":"Japan","name":"Yen","longname":"Japanese Yen"},
			...
			function set_curr_detail(){
				var s = get_selected_currency_symbol();
			...
			function set_dc_cookie(){
				dc = get_selected_currency_symbol();
				Cookies.set('btc_widget_dc', dc, { expires: 7 });
			}
		...
		<?php 
		if(isset($_COOKIE['btc_widget_dc'])){
			$dc = $_COOKIE['btc_widget_dc'];
		}
		else{
			$dc = 'USD';
		}
		...
		?>
			...
			<span id="up" style="white-space:nowrap;"><span id="cur_symbol"><strong style="font-size:1.2em;position:relative;margin-right:.1em"><?= $dc ?></strong></span> <input type="text" name="cur_amt" id="cur_amt" data-autosize-input='{ "space": 0 }' value="<?= number_format((($j->$dc->{'last'}*$instance['default_value'])),2,'.','') ?>" /></span><br>
			...
			foreach(array_keys($o) as $c){
				if(($c!='timestamp') && ($o[$c]->{'last'}>0)){
					?>
					<option data-symbol="<?= $c ?>" value="<?= number_format($o[$c]->{'last'},2,'.','') ?>" <?php if($c==$dc){echo 'selected="selected"';} ?>><?= $c ?></option>
					<?php
				}
			}
		...
		?>
			...
			<span id="up"><span id="cur_symbol"><strong><?= $dc ?></strong></span> <input type="text" name="cur_amt" id="cur_amt" data-autosize-input='{ "space": 0 }' value="<?= number_format((($j->$dc->{'last'}*$instance['default_value'])),2,'.','') ?>" /></span><br>
			<small><span id="cur_long_name"></span></small><br>
			<select name="currency" id="crsel"><?php
			foreach(array_keys($o) as $c){
				if(($c!='timestamp') && ($o[$c]->{'last'}>0)){
...
?>
	<script type="text/javascript">
		...
		function set_curr_detail(t){
			var s = t.data('symbol');
			jQuery('#cur_long_name').html(currenciesJSON[s].longname);
			jQuery('#cur_symbol').html('<strong style="font-size:1.2em;position:relative;margin-right:.1em">'+currenciesJSON[s].symbol+'</strong>');
		}
		function update_exc(t){
			var s = t.data('symbol');	
			var v = jQuery('#crsel').val();
			var b = jQuery('#btc_amt').val();
			v = v*b;
